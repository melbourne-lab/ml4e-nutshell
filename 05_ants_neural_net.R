...
#' ants data and the `keras` package. You'll need to install Python and
#' Tensorflow to use the `keras` package. The `keras` package is an R interface
#' to the Python Keras library, which in turn is an interface to the Python
#' Tensorflow library, which in turn is an interface to Tensorflow (mostly C++)!
#' See 05_keras_installation.md. The Python Keras library is widely used and the
...
#' We want to see the training error (MSE, labelled "loss" above) decline to a
#' reasonable level. Although the error will continue to go down, here we see it
#' leveling out somewhat at about MSE=7, which is an absolute error of sqrt(7) =
#' +/- 2.6 species.
#' 
